...
system paper~\cite{andreakis2020dblog}. This document accompanies the
development as bundled in the Dual\_Write\_Recovery deposit for its
worked instance; the development's published archival record is
version~2.1 on Zenodo~\cite{andreakis2026artifact}, with its frozen
predecessors separately archived (v2.0 at DOI 10.5281/zenodo.20652511,
v1.0 at DOI 10.5281/zenodo.20389697). The
development is distributed under the BSD 3-Clause license.

\subsection*{Trust base}

The session contains no \isa{axiomatization}, \isa{typedecl}, or
\isa{consts} declaration. The single \isa{typedef} introduces the
source-coordinate type as a copy of \isa{nat} (a conservative
extension over a provably nonempty set), with its \isa{linorder} and
...
Layer & Theories \\
\midrule
0 (source/replay substrate) & \isa{Source{\isacharunderscore}History}, \isa{Replay}, \isa{Scope}, \isa{Source{\isacharunderscore}Coordinates} \\
...
witnesses/fixtures & the \isa{{\isacharunderscore}Core}/\isa{{\isacharunderscore}Model}/\isa{{\isacharunderscore}Inst} families \\
